...
void ThirdStage::mUpdate() {
	...
	if (CheckHitKey(KEY_INPUT_P) == 0 && charChangeFlag == true) {
		...
		if (gameManager->GetCurrentCharType() == e_charType::eRect_Blue) {
			...
			gameManager->ChangeCharType(e_charType::eRect_Red);
		}
		else if (gameManager->GetCurrentCharType() == e_charType::eRect_Red) {

			gameManager->ChangeCharType(e_charType::eRect_Green);
		}
		else {
			gameManager->ChangeCharType(e_charType::eRect_Blue);
		}
	...
	if (gameManager->GetRectGoalFlag(eColor_Red) == true && gameManager->GetRectGoalFlag(eColor_Blue) == true && gameManager->GetRectGoalFlag(eColor_Green) == true)
		gameManager->SetGameClearFlag(true);

}

void ThirdStage::mRender() {
	STAGE_DOUBLE_LOOP{
		if (StageObject != nullptr)
		StageObject->mRender();
	} LOOP_END;
